...
public class roombehavior : NetworkBehaviour {
	...
	public void CmdCreateRoom()
	{
		...
		Vector3 oldposition = this.gameObject.transform.localPosition;
		Vector3 newposition = new Vector3 (oldposition.x - 60, oldposition.y, oldposition.z);
		var newRoom = (GameObject)Instantiate(roomPrefab, newposition, Quaternion.identity);

		NetworkServer.Spawn (newRoom);
	}

	[ClientRpc]
	public void RpcRemoveTriggerPlane()
	{
		Destroy (GetComponentInChildren<trigger>());
	}
}
